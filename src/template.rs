...
impl Template {
    pub(crate) async fn parse_template(
        ...
        let mut file = Box::pin(read_from
            .read_from(path))
            .await
    ...
    async fn parse_external_template<R: Reader>(
        ...
        parse_tag!(parser => &mut buffer, "template", {
            "object" => for attrs {
                object = Box::new(Some(Box::pin(ObjectData::new(parser, attrs, Some(&tileset_gid), tileset.clone(), parent(template_path).ok_or(Error::PathIsNotFile)?, read_from, cache)).await?));
                Ok(())
            },
            "tileset" => for attrs {
                let res = Box::pin(Tileset::parse_xml_in_map(parser, &attrs, template_path, read_from, cache)).await?;
